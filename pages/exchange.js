...
export async function getServerSideProps({ query }) {
  // Extract the defaultCurrencyCode from the query object
  const { defaultCurrencyCode } = query

  // Add a default value for defaultCurrencyCode in case it is not provided in the query
  const currencyCode = defaultCurrencyCode || "USD"

  const response = await api().get(`/v6/${API_KEY}/latest/${currencyCode}`)
...
export default function Home({ currencies, defaultCurrencyCode }) {
  ...
  return (
    ...
    >
      <Head>
        <title>Curren See</title>
        <meta
          name="description"
          content="Curren See is a web application that allows users to perform currency conversions between different currencies based on real-time exchange rates. The app is built using Next.js and Chakra UI for a modern and user-friendly experience."
        />
